...
fn ninja_featureless_plan_is_byte_stable_v0() {
    ...
    // …and the whole file is pinned byte-for-byte to the v0 emission. If
    // this golden changes, every user's first post-upgrade build relinks the
    // world; change it only deliberately.
    let expected = "\
...
rule link_cxx
  command = /usr/bin/c++ $link_flags -o $out $in $libs
  description = LINK $out

rule link_c
  command = /usr/bin/cc $link_flags -o $out $in $libs
  description = LINK $out
